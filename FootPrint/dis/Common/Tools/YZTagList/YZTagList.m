...
// 保存点击标签
- (void)saveClickTag:(NSString *)tagStr
{
    // 获取对应的标题按钮
    YZTagButton *button = self.tags[tagStr];
    ...
    if (button.selected == YES) {
        [button setTitleColor:RGBA(50,207,225, 1.0) forState:UIControlStateNormal];
        button.layer.borderColor = RGBA(50,207,225, 1.0).CGColor;
    }else{
        button.layer.borderColor = _borderColor.CGColor;
        [button setTitleColor:_tagColor forState:UIControlStateNormal];
    }
}
