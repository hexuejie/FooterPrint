    ...
    for (int i=0; i<rowCount+1; i++) {
        if ([_ShowHorizonLine[i] integerValue]>0) {
            if(i == rowCount){
                ...
                shapeLayer.strokeColor = [[[UIColor whiteColor] colorWithAlphaComponent:1] CGColor];
...
//    }else
    ...
    for (int i=0; i<xLabels.count+1; i++) {
        CAShapeLayer *shapeLayer = [CAShapeLayer layer];
        UIBezierPath *path = [UIBezierPath bezierPath];
        [path moveToPoint:CGPointMake(UUYLabelwidth+i*_xLabelWidth+_xLabelWidth/2,UULabelHeight)];
        [path addLineToPoint:CGPointMake(UUYLabelwidth+i*_xLabelWidth+_xLabelWidth/2,self.frame.size.height-2*UULabelHeight)];
        [path closePath];
        shapeLayer.path = path.CGPath;
        shapeLayer.strokeColor = [[[UIColor whiteColor] colorWithAlphaComponent:1] CGColor];
        shapeLayer.fillColor = [[UIColor whiteColor] CGColor];
...
//                [progressline stroke];
        ...
        pathAnimation.duration = 1;//childAry.count*0.05;
