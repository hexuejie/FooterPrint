...
#if YCDownload_Mgr_Item
//items
static const char* allItemKeys[] = {"fileId", "taskId", "downloadURL","downloadCid", "uid", "fileType", "fileExtension", "rootPath", "fileSize", "downloadedSize", "downloadStatus", "extraData", "version", "createTime", "enableSpeed"};
...
+ (void)initDatabase{
    ...
    NSString *sql = @"CREATE TABLE IF NOT EXISTS downloadItem (pid integer PRIMARY KEY AUTOINCREMENT,taskId text not null unique,fileId text, downloadURL text,downloadCid text,uid text,fileType text,fileExtension text,rootPath text,fileSize integer,downloadedSize integer,downloadStatus integer,extraData BLOB, version text not null, createTime integer); \n"
    "CREATE TABLE IF NOT EXISTS downloadTask (pid integer PRIMARY KEY AUTOINCREMENT,taskId text not null unique, downloadURL text,downloadCid text, stid integer, priority float, enableSpeed integer, fileSize INTEGER, downloadedSize INTEGER, version text not null, tmpName text, resumeData BLOB, extraData BLOB, createTime integer);";
...
+ (NSArray *)itemsWithCid:(NSString *)downloadCid uid:(NSString *)uid{
    NSMutableArray *items = [NSMutableArray array];
    [self performBlock:^BOOL{
        NSString *sql = [NSString stringWithFormat:@"select * from downloadItem where downloadCid == '%@' and uid == '%@'", downloadCid, uid];
        NSArray *rel = [self selectSql:sql];
        [rel enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            YCDownloadItem *item = [self itemWithDict:rel[idx]];
            if(item) [items addObject:item];
        }];
