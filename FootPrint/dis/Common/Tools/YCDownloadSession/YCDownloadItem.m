...
- (instancetype)initWithUrl:(NSString *)url fileId:(NSString *)fileId downloadCid:(NSString *)downloadCid {
    if (self = [self initWithPrivate]) {
        _downloadURL = url;
        _fileId = fileId;
        _downloadCid = downloadCid;
    }
    return self;
}
...
+ (instancetype)itemWithUrl:(NSString *)url fileId:(NSString *)fileId downloadCid:(NSString *)downloadCid {
    
    return [[YCDownloadItem alloc] initWithUrl:url fileId:fileId downloadCid:downloadCid];
}
...
- (void)downloadStatusChanged:(YCDownloadStatus)status downloadTask:(YCDownloadTask *)task {
    ...
    if (status == YCDownloadStatusFinished) {
        ...
        CoursePlayerFootModel *model = [CoursePlayerFootModel infoWithData:self.extraData];
        [KeyWindow showTip:[NSString stringWithFormat:@"%@下载完成",model.title]];
        NSLog(@"沙盒路径========%@",self.savePath);
    }
...
- (void)setFileExtensionWithTask:(YCDownloadTask *)task {
    ...
    if ([oriResponse isKindOfClass:[NSHTTPURLResponse class]]) {
        ...
        if ([task.downloadURL containsString:@"mp3"]) {
            extension = @"mp3";
        }else if ([task.downloadURL containsString:@"mp4"]){
            extension = @"mp4";
        }
...
- (NSString *)saveName {
    ...
    saveName = [saveName stringByAppendingPathExtension: self.fileExtension.length>0 ? self.fileExtension : @"data"];
    return saveName;
}

- (NSString *)savePath {
    
    NSString *path = [[self saveDirectory] stringByAppendingPathComponent:[self saveName]];
    return path;
}

- (NSString *)description {
    return [NSString stringWithFormat:@"<YCDownloadTask: %p>{taskId: %@, url: %@ fileId: %@ downloadCid: %@}", self, self.taskId, self.downloadURL, self.fileId, self.downloadCid];
}
