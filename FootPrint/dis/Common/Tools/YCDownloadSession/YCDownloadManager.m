...
+ (NSArray *)itemsWithDownloadCid:(NSString *)downloadCid{
    return [YCDownloadMgr itemsWithDownloadCid:downloadCid];
}
...
- (void)startDownloadWithUrl:(NSString *)downloadURLString fileId:(NSString *)fileId  priority:(float)priority extraData:(NSData *)extraData {
    YCDownloadItem *item = [YCDownloadItem itemWithUrl:downloadURLString fileId:fileId downloadCid:@"44"];
...
- (NSArray *)itemsWithDownloadCid:(NSString *)downloadCid {
    return [YCDownloadDB itemsWithCid:downloadCid uid:self.uid];
}
