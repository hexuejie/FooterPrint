...
-(void)jk_showInWindowWithMode:(JKCustomAnimationMode)animationMode inView:(UIView *)superV viewFrame:(CGRect)frame bgAlpha:(CGFloat)alpha needEffectView:(BOOL)isNeed{
...
-(void)upToDownShowInWindow{
    ...
    [UIView animateWithDuration:animationTime delay:0 usingSpringWithDamping:1 initialSpringVelocity:5 options:UIViewAnimationOptionCurveEaseIn animations:^{
...
-(void)dropDown{
    if (self.superview) {
        [UIView animateWithDuration:animationTime delay:0 usingSpringWithDamping:1 initialSpringVelocity:3 options:UIViewAnimationOptionCurveEaseOut animations:^{
            self.frame = CGRectMake(self.frame.origin.x, -viewFrame.size.height, self.frame.size.width, self.frame.size.height);
        } completion:^(BOOL finished) {
            [self hideAnimationFinish];
        }];
    }
