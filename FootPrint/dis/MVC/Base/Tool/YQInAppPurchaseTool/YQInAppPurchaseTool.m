...
- (void)verifyPruchaseWithID:(NSString *)ProductID
{
    ...
    NSString *checkURL;
    if ([isAudit isEqualToString:@"no"]) {
        checkURL = productCheckURL;
    } else {
        checkURL = sandboxCheckURL;
    }
    ...
    if (dict != nil) {
        ...
        if ([dict[@"status"] integerValue] == 21007) { //沙盒环境，需要在进行一次沙盒验证
            
            [self verifyPruchaseSandboxWithID:ProductID];
        }else{
...
- (void)verifyPruchaseSandboxWithID:(NSString *)ProductID{
