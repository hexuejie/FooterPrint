...
- (instancetype)initWithFrame:(CGRect)frame type:(ProgressBarType)type
{
    ...
    if (self) {
        ...
        if (type == loopProgressBar) {
            CAShapeLayer *layer = [CAShapeLayer layer];
            layer.frame = self.bounds;
            layer.cornerRadius = MIN(layer.bounds.size.width, layer.bounds.size.height) / 2.0;
            ...
            layer.path = loopPath.CGPath;
            layer.lineWidth = 2.0;
            layer.strokeStart = 0;
            layer.strokeEnd = 1;
            layer.fillColor = [UIColor clearColor].CGColor;
            layer.strokeColor = [UIColor colorWithRed:238/255.0 green:238/255.0 blue:238/255.0 alpha:1].CGColor;
...
- (void)initializeUserInterface
{
    if (self.type == progressBar) {//条形进度条
        ...
        if (self.type == loopProgressBar) {//环形进度条
            [self.baseView.layer addSublayer:self.outerRingLayer];
        }
