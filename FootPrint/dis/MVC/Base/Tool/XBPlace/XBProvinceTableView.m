...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XBProvinceObj *provinceObj = _provinceArray[indexPath.section];
    XBCityObj *obj = provinceObj.cities[indexPath.row];
...
//    if (![obj isKindOfClass:[XBCityObj class]]) {
        if ([self.myDelegate respondsToSelector:@selector(provinceTableView:didSelectedAreaCell:areaName:)]) {
            [self.myDelegate provinceTableView:self didSelectedAreaCell:indexPath areaName:[NSString stringWithFormat:@"%@%@",provinceObj.name,obj.name]];
        }
    if ([self.myDelegate respondsToSelector:@selector(provinceTableView:didSelectedAreaCell:CityName:)]) {

        [self.myDelegate provinceTableView:self didSelectedAreaCell:indexPath CityName:[NSString stringWithFormat:@"%@",obj.name]];
    }
//        return;
//    }
//    // 走到这里说明点击的是市
