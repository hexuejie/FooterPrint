...
#import "TalkfunPictureBrowseView.h"
...
@interface TalkfunPictureBrowseView () <UIScrollViewDelegate>
...
@implementation TalkfunPictureBrowseView
...
- (void)setupImageView:(NSMutableArray *)array{
    ...
    for (TalkfunPhotoZoom *photoZoom in self.scrollView.subviews) {
    ...
    for (int i = 0; i < array.count ; i++) {
        ...
        TalkfunPhotoZoom * photoZoomView = [[TalkfunPhotoZoom alloc] initWithFrame:CGRectMake(i * self.frame.size.width, 0, self.frame.size.width, self.frame.size.height)];
    ...
    self.currentPhotoZoom = (TalkfunPhotoZoom *)(self.scrollView.subviews.firstObject);
...
- (void)setImage:(UIImage*)image
{
    ...
    TalkfunPhotoZoom * photoZoomView = [[TalkfunPhotoZoom alloc] initWithFrame:CGRectMake(0 * self.frame.size.width, 0, self.frame.size.width, self.frame.size.height)];
    photoZoomView.imageNormalWidth = self.frame.size.width;
    photoZoomView.imageNormalHeight = self.frame.size.height;
    ...
    [photoZoomView.imageView showGifImageWithData:imageData];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if (scrollIndex != self.index) {
        //重置上一个缩放过的视图
        TalkfunPhotoZoom * zoomView  = (TalkfunPhotoZoom *)scrollView.subviews[self.index];
        [zoomView pictureZoomWithScale:1.0];
        
        self.index = scrollIndex;
        self.currentPhotoZoom = (TalkfunPhotoZoom *)scrollView.subviews[self.index];
